...
/*
	...
	gswitch.subtype = sSwitchTypeSelector;
	gswitch.id = ChildID;
	gswitch.unitcode = NodeID;
	gswitch.cmnd = selectorValue;
	gswitch.level = 100;
...
/*
	unsigned long long devidx = DeviceRowIdx=m_sql.UpdateValue(m_HwdID, szTmp, 1, pTypeGeneralSwitch, sSwitchTypeSelector, 12, 255, 0, "0", devname);
	if (devidx != -1)
	{
		//Set switch type to selector
		m_sql.safe_query("UPDATE DeviceStatus SET SwitchType=%d WHERE (ID==%llu)", STYPE_Selector, devidx);
		//Set default device options
		m_sql.SetDeviceOptions(devidx, m_sql.BuildDeviceOptions("SelectorStyle:12;LevelNames:Arret|Eco|Confort", false));
	}
*/
}
